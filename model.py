...
print 'Successfully imported test data'
...
with open('extra_images_greyscale.pickle', 'rb') as pickled_data_train:
    extra_data_dict = pickle.load(pickled_data_train)

assert(len(extra_data_dict) == extra_data_rows)
print 'Successfully imported extra data'
...
def display_random_image(ds, labels):
    ...
    #im = Image.fromarray(np.multiply(ida, 255))
...
for i in range(len(extra_labels)):
    if len(extra_labels[i]) != 5:
        print i, extra_labels[i]
        et_labels.append(np.array(extra_labels[i][:5]))
    else:
        et_labels.append(np.array(extra_labels[i]))
et_labels = np.array(et_labels)
del extra_labels
extra_labels = et_labels
del et_labels
